...
```ts
const rich = new MsdfLabel(
    '支持 <font color="#f9e38f" size="36">HTML</font>、<i>斜体</i>、<u>下划线</u>'
);
rich.html = true;
...
```bash
tsx scripts/generate-msdf-font.ts \
...
- `scripts/generate-msdf-font.ts` now post-processes the atlas/json and injects a dedicated decoration glyph for underline and strikethrough. You do not need to keep that glyph in the charset file.
